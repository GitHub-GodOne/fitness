...
export async function POST(req: Request) {
  try {
    ...
    const files = Array.isArray(body.files) ? (body.files as PresignFileInput[]) : [];
    ...
    for (const file of files) {
      ...
      const ext = extFromMime(fileType) || path.extname(fileName).replace('.', '') || 'bin';
      const safeBaseName =
      ...
      if (!signedUpload.success || !signedUpload.url) {
        return respErr(
          signedUpload.error || 'Storage provider does not support direct uploads',
          501
        );
      }
