...
export async function POST(req: Request) {
  try {
    ...
    console.log("paymentProviderName----------->", paymentProviderName);
    ...
    console.log("paymentProvider2----------->", paymentProvider);
    ...
    console.log("paymentProductId---------->11111111111111111", paymentProductId);
    ...
    if (!paymentProductId) {
      ...
      console.log("paymentProductId---------->77777777777777", paymentProductId);
    }
    ...
    if (!paymentProductId) {
      console.log("paymentProductId---------->33333333333", paymentProductId);
      // checkout price validation
      if (!checkoutPrice.amount || !checkoutPrice.currency) {
        return respErr('invalid checkout price');
      }
    } else {
      console.log("paymentProductId---------->444444444444", paymentProductId);
    ...
    console.log("paymentProductId---------->22222222222", paymentProductId);
    ...
    try {
      console.log("result-----------> 到这里了哦哈哈哈哈！！！！！！！！ojbk");
      // create payment
      const result = await paymentProvider.createPayment({
        order: checkoutOrder,
      });
      console.log("result-----------> 到这里了哦哈哈哈哈！！！！！！！！", result);
...
async function getPaymentProductId(
  ...
  try {
    ...
    console.log("productIds---------->88888888888888", creemProductIds);
    if (creemProductIds) {
      const productIds = JSON.parse(creemProductIds);
      console.log("productIds---------->88888888888888", productIds, `${productId}_${checkoutCurrency}`, productId);
      return (
